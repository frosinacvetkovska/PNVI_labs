...
WINWIDTH = 960 # width of the program's window, in pixels #requirement 1 change width of camera
...
CAMERASLACK_X = 120  # requirement 2 horizontal direction of the camera
CAMERASLACK_Y = 90   # requirement 2 vertical direction of the camera
...
LOSTSIZE = 15        # requirement 4: how big the player need to lose
...
NUM_OBSTACLES = 5    # requirement 5
MAX_OBSTACLES = 15   # requirement 5
OBSTACLES_SIZE =15   # requirement 5
LEFT = 'left'
RIGHT = 'right'
...
def runGame():
    ...
    playerObj = {'surface': pygame.transform.scale(L_SQUIR_IMG, (STARTSIZE, STARTSIZE)),
                 ...
                 'bounce':0,
                 'health': MAXHEALTH,
                 'last_obstacle_time': time.time()} #requirement 5
    ...
    while True: # main game loop
        ...
        #requirement 2: change of CAMERASLACK to CAMERASLACK_X and CAMERASLACK_Y:
        ...
        #requirement 5: add obstacle after a certain time
        if time.time() - playerObj['last_obstacle_time'] > random.randint(5, 15):
            for _ in range(min(NUM_OBSTACLES, MAX_OBSTACLES - len(obstacles))):
                obstacle = {'x': random.randint(0, WINWIDTH - OBSTACLES_SIZE),
                             'y': random.randint(0, WINHEIGHT - OBSTACLES_SIZE),
                             'rect': pygame.Rect(0,0, OBSTACLES_SIZE, OBSTACLES_SIZE)}
                obstacles.append(obstacle)
            playerObj['last_obstacle_time'] = time.time()
        #requirement 5: check if player collides with any obstacle
        for obstacle in obstacles:
            if playerObj['rect'].colliderect(obstacle['rect']):
                playerObj['health'] -=1
        ...
        drawHealthMeter(playerObj['health'])
         ...
         #requirement 5: draw the obstacle
        for obstacle in obstacles:
            obstacle['rect'] = pygame.Rect(obstacle['x'] - camerax, obstacle['y'] - cameray, OBSTACLES_SIZE, OBSTACLES_SIZE)
            pygame.draw.rect(DISPLAYSURF, RED, obstacle['rect'])
...
def makeNewSquirrel(camerax, cameray):
    ...
    if random.choice(direction) == 1:
        ...
        sq['bounceheight'] = random.randint(-50,-10)
    return sq
