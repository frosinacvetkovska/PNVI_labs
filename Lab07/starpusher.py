...
def main():
    ...
    while True: # main game loop
        ...
        if result in ('solved', 'next'):
            ...
            if currentLevelIndex >= len(levels) or currentLevelIndex == previousLevelIndex:
...
def startScreen():
    ...
    instructionText = ['Push the stars over the marks.',
                       'WASD to move, Arrow keys for camera control, P to change character.',
    ...
    for x in range(len(mapObj)):
        for y in range(len(mapObj[x])):
            ...
            if mapObj[x][y] in OUTSIDEDECOMAPPING:
                ...
                #requirement 3
                mapSurf.blit(pygame.image.load(random.choice(['Star.png', 'Star1.png', 'Star2.png'])), spaceRect)
            elif (x, y) in goals:
